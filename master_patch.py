...
# 4. Inject get_live_llm_summary call and fix generator exhaustion in render_dashboard_preview
if "llm_summary = get_live_llm_summary" not in content:
    content = content.replace(
        '    model = build_preview_model(events)\n',
        '    events = list(events)\n    model = build_preview_model(events)\n    llm_summary = get_live_llm_summary(model.get("recent_events", events))\n'
    )

# 5. Inject the HTML into the dashboard
if "{_ai_summary_panel(llm_summary)}" not in content:
    content = content.replace(
        '<section class="grid">',
        '{_ai_summary_panel(llm_summary)}\n    <section class="grid">'
    )

# 6. Fix the write_dashboard_preview caching deadlock
cache_replacement = """    changed = False
    ...
    # Force redraw if LLM cache updated
...
if "Force redraw if LLM cache updated" not in content:
    content = re.sub(
        r'    changed = False\n    if current_file_states != _LAST_PREVIEW_STATE\.get\("file_states"\):\n        changed = True\n\n    output = Path\(output_path\)\n    if not changed and output\.exists\(\):\n        return current_file_states',
        cache_replacement + '\n\n    output = Path(output_path)',
        content
